...
from database import init_db, add_employee, update_employee, get_all_employees, get_all_employees_no_blob, delete_employee, mark_attendance, get_attendance_logs, get_attendance_logs_count, update_attendance_time, get_db_connection, get_cursor, get_placeholder, sync_attendance_to_supabase, delete_attendance_record
...
def last_activity():
    ...
    try:
        # Get the single most recent activity (order by date then latest time)
        # Note: We check both login_time and logout_time to find the absolute latest entry
        cursor.execute(f"""
            SELECT e.name, a.login_time, a.logout_time, a.date
            FROM attendance a 
            JOIN employees e ON a.employee_id = e.employee_id 
            WHERE a.login_time != 'Absent' AND a.login_time != ''
            ORDER BY a.date DESC, a.login_time DESC, a.logout_time DESC LIMIT 1
        """)
        row = cursor.fetchone()
        
        if row:
            # Determine if the latest action was a logout or login
            status = "Logged In"
            time = row['login_time']
            if row['logout_time'] and row['logout_time'] != '---' and row['logout_time'] != '':
                status = "Logged Out"
